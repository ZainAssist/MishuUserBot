...
async def dmraid(xspam: Client, e: Message):
      """ Module: Dm Raid """
      RishuUserBot = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
      if len(MishuUserBot) == 2:
          ok = await xspam.get_users(MishuUserBot[1])
          id = ok.id
...
async def dmspam(spam: Client, e: Message):
      ...
      RishuUserBot = text[1:]
      if len(RishuUserBot) == 2:
          msg = str(MishuUserBot[1])
