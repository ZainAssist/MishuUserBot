...
from MishuUserBot.database.pmpermitdb import get_approved_users, pm_guard
import MishuUserBot.database.pmpermitdb as MishuUserBot
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await MishuUserBot.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await MishuUserBot.set_block_message(RishuUserBot.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await MishuUserBot.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await MishuUserBot.get_pm_settings()
    await MishuUserBot.allow_user(chat_id)
...
async def deny(client, message):
    chat_id = message.chat.id
    await MishuUserBot.deny_user(chat_id)
...
async def reply_pm(app: Client, message):
    global FLOOD_CTRL
    pmpermit, pm_message, limit, block_message = await RishuUserBot.get_pm_settings()
